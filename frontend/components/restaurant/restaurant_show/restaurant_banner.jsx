...
const RestaurantBanner = props => {
  const { restaurant, deleteRestaurant, currentUser, deleteFavorite, createFavorite } = props;
  ...
  const manageFavoriteOptions = () => {
    let result;
    if (currentUser && restaurant.favorite_ids) {
      if (currentUser.favorite_ids.some(el => restaurant.favorite_ids.includes(el))) {
  ...
  const manageRestaurantOptions = () => {
    let result;
    currentUser && currentUser.id === restaurant.owner_id ?
      result = (
        <div className="button-container">
          <button
            className="readon danger"
